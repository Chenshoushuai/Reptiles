...
def compute_bic(mfcc_v,delta):#BIC算法
    m, n = mfcc_v.shape
    # print(m, n)
...
def speech_segmentation(mfccs):
    #初始化检测窗[wStart,wEnd] 分割多个节点
    wStart = 0
    wEnd = 200
    wGrow = 200 #不存在分割点，不移动检测窗口的位置，调整wEnd
    delta = 25
    #print(mfccs)
...
def multi_segmentation(file,sr,frame_size,frame_shift,plot_seg = False): #plot_seg默认值是False
    ...
    #n_fft为enframe的帧长度(采样点)，hop_length为帧移的长度，n_mfcc为特征的维数。
   ...
   # print(seg_point)
    seg_point = np.append(seg_point, len(y)) #y的长度插入到数组的最后面
   # print(seg_point)
    ...
    #print(np.asarray(output_segpoint) / float(sr))
    '''if plot_seg: #画图的部分
